...
import sys
J_wdir = "/home/wanjiz/RegionCLIP"
#sys.path =[x for x in sys.path if x != J_wdir]
#if "/home/ahmad/region_clip" not in sys.path:
#    sys.path.append( "/home/ahmad/region_clip" )
import argparse
import json
import numpy as np
import os
...
def create_instances(predictions, image_size):
    ...
    all_scores=  np.asarray([x["all_scores"] for x in predictions])
    ...
    ret.all_scores = all_scores[:len(score),:-1]
...
if __name__ == "__main__":
    ...
    for dic in tqdm.tqdm(dicts):
        ...
        vis_pred = vis.draw_instance_predictions_prob(predictions).get_image()

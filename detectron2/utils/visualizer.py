...
class Visualizer:
    ...
    def overlay_instances_rpn(
        ...
        *,
        ...
        if assigned_colors is None:
            assigned_colors = [random_color(rgb=True, maximum=1) for _ in range(num_instances)]
        #if assignment colors don't match then choose first in array
        elif len(assigned_colors) != num_instances:
    ...
    def draw_box_rpn(self, box_coord, alpha=0.5, edge_color="g", line_style="-",linewidth=None):
        """
        Args:
            box_coord (tuple): a tuple containing x0, y0, x1, y1 coordinates, where x0 and y0
                are the coordinates of the image's top left corner. x1 and y1 are the
        ...
        #linewidth = max(self._default_font_size / 4, 1)
        #linewidth = 1 #predefine smaller width
